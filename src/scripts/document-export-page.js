...
H5P.DocumentExportPage = (function ($, EventDispatcher) {
  ...
  DocumentExportPage.prototype.attach = function ($container) {
    ...
    if (
      self.params.helpText !== undefined &&
      self.params.helpText.length !== 0
    ) {
  ...
  DocumentExportPage.prototype.initDocumentExportButton = function () {
    var self = this;
    H5P.DocumentationTool.handleButtonClick(
      self.$exportDocumentButton,
      function () {
        // Check if all required input fields are filled
        if (self.isRequiredInputsFilled()) {
          var exportDocument = new H5P.DocumentExportPage.CreateDocument(
            self.params,
            self.exportTitle,
            self.submitEnabled,
            self.inputArray,
          ...
          //Trigger XAPI submit on "Create Document" button click
   ...
   */
  DocumentExportPage.prototype.updateRequiredInputsFilled = function (
    pageTitles
  ) {
