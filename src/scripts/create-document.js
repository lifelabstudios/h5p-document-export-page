...
H5P.DocumentExportPage.CreateDocument = (function ($, EventDispatcher) {
  ...
  function CreateDocument(
    ...
    this.hasAssessedGoals = this.inputGoals.inputArray.some(function (
      inputGoalPage
    ) {
  ...
  CreateDocument.prototype.attach = function ($container) {
    ...
    var exportPage = new H5P.DocumentExportPage.ExportPage(
      this.title,
      exportString,
      this.submitEnabled,
  ...
  CreateDocument.prototype.createExportableGoalsList = function () {
    if (
      this.inputGoals === undefined ||
      !this.inputGoals.inputArray ||
      this.inputGoals.inputArray.length === 0
    ) {
      return;
    }
        ...
        inputGoalPage.forEach(function (inputGoal) {
          ...
          if (!goalCategoryExists) {
            goalsList.push({
              label: "",
              goalArray: [],
              goalAnswer: inputGoal.goalAnswer(),
            });
          ...
          if (
            inputGoal.goalText().length &&
            inputGoal.getTextualAnswer().length
          ) {
    ...
    this.inputFields.forEach(function (inputFieldPage) {
      if (inputFieldPage.inputArray && inputFieldPage.inputArray.length) {
        ...
        inputFieldPage.inputArray.forEach(function (inputField) {
          if (inputField.plainText) {
            standardPage.inputArray.push({
              plainText: inputField.plainText,
            });
          } else {
            standardPage.inputArray.push({
      ...
      if (inputPage.inputArray && inputPage.inputArray.length) {
        ...
        inputPage.inputArray.forEach(function (inputInstance) {
          if (inputInstance) {
            ...
            if (inputInstance.plainText) {
              inputParagraph.append(
                $("<strong>", {
                  html: inputInstance.plainText,
                })
              );
            }
  ...
  CreateDocument.prototype.createGoalsOutput = function () {
    if (
      this.exportableGoalsList === undefined ||
      this.exportableGoalsList.length === 0
    ) {
      return "";
    }
