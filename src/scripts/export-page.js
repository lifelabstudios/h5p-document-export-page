...
H5P.DocumentExportPage.ExportPage = (function ($, EventDispatcher) {
  function ExportPage(
    ...
    var exportPageTemplate =
      ...
      '     <button class="joubel-exportable-copy-button" title ="' +
    ...
    self.$exportableArea = $(".joubel-exportable-area", self.$inner).append(
      $bodyReplacedLineBreaks
    );

    self.initExitExportPageButton();
    self.actLikeSubmitButtonOnClick();
  ...
  ExportPage.prototype.actLikeSubmitButtonOnClick = function () {
    var self = this;
    self.$successDiv = $("<div/>", {
      text: self.standardSubmitSuccessTextLabel,
      class: "joubel-exportable-success-message",
  ...
  ExportPage.prototype.resize = function () {
    ...
    var leftMargin = parseInt(
      $(".joubel-exportable-header-text", $headerInner).css("font-size"),
      10
    );
    var rightMargin = parseInt(
      $(".joubel-export-page-close", $headerInner).css("font-size"),
      10
    );

    var dynamicRemoveLabelsThreshold = this.calculateHeaderThreshold(
      $innerTmp,
    ...
    var dynamicRemoveTitleThreshold = this.calculateHeaderThreshold(
      $innerTmp,
      leftMargin + rightMargin
    );
   ...
   */
  ExportPage.prototype.calculateHeaderThreshold = function (
    $container,
    margin
  ) {
    var staticPadding = 1;
    ...
    let buttonWidth = $submitButtonTmp.length
      ? $submitButtonTmp.outerWidth()
      : 0;
    buttonWidth += $selectTextButtonTmp.length
      ...
      self.templateContent.flatInputList.forEach(function (content) {
        ...
        if (content.inputArray) {
          content.inputArray.forEach(function (field) {
            if (field.plainText) {
              const plainText = field.plainText.split("\n").map(
                (line) =>
                  new TextRun({
                    break: 1,
                    text: line.replace(/(\r\t|\t|\r)/gm, ""),
                    bold: false,
                    size: 24,
                  })
              );
